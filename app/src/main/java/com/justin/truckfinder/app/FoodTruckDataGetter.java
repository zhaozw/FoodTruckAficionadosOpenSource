...
//                    Toast.makeText(context, "Updated Food Trucks with more information.", Toast.LENGTH_SHORT).show();
    ...
    private static void detailsSearchGooglePlaces(FoodTruckData partialFoodTruck) {
        ...
        String myAPIGoogleDetails = "ERROR";
        StringBuilder stringBuilderGoog = new StringBuilder(myAPIGooglePartial);
        ...
        try {
            ...
            myAPIGoogleDetails = stringBuilderGoog.toString();
            if(myAPIGoogleDetails.contains("+%26")) {
                myAPIGoogleDetails = myAPIGoogleDetails.replace("+%26","");
            }
            if(myAPIGoogleDetails.contains("%21")){
                myAPIGoogleDetails = myAPIGoogleDetails.replace("%21","");
            }
            if(myAPIGoogleDetails.contains("%27")){
                myAPIGoogleDetails = myAPIGoogleDetails.replace("%27",""); // %27 = '
            }
            if(myAPIGoogleDetails.contains("Parking")){
                return;
            }
            if(myAPIGoogleDetails.contains("Poop")){
                return;
            }
        ...
        try {
            CustomJSONRequest jsonObjectRequest = new CustomJSONRequest(Request.Method.GET,
                    myAPIGoogleDetails,
                    null,
                    createMyGooglePlaceDetailsReqSuccessListener(),
                    errorListener);
    ...
    private static Response.Listener<JSONObject> createMyGooglePlaceDetailsReqSuccessListener() {
        return new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
